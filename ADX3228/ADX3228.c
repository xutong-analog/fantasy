...
void ADX3228_WriteMultiReg(uint8_t StartReg,uint8_t RegNum,uint8_t *Data)
{
    ...
    uint8_t i;
    //写入命令，
    TxData[0]=ADX3228_WREG|StartReg;
    //选择要写入的的寄存器
    TxData[1]=RegNum-1;
    for(i=0;i<RegNum;i++)
    {
...
void ADX3228_ReadMultiReg(uint8_t StartReg,uint8_t RegNum,uint8_t *Data)
{
    ...
    uint8_t TxData[3];
    //读取命令，
    TxData[0]=ADX3228_RREG|StartReg;
    //选择要写入的的寄存器
    TxData[1]=RegNum-1;
    //最终接收多少个数据
...
//发送ADX3228 数据指令 如SDATAC或者RDATAC
